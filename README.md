...
Evidence-driven Abaqus/CAE mesh replacement and coupled thermal–stress diagnosis for Codex.

`abaqus-skill` is a reusable Codex skill for updating an existing Abaqus model after remeshing. It treats the original `.cae` as the authoritative source of analysis settings and the new `.inp` as the mesh source, then validates the replacement through CAE reopening and generated-input audits.
...
The contract can verify step order, time and increment controls, body heat flux, temperature mappings, required keyword patterns, and forbidden importer-generated names. See `references/mesh-replacement-patterns.md` for an example contract.
